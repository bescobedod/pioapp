// Source - https://stackoverflow.com/a/79139193
// Posted by Bruno Pintos
...
const getGoogleDriveUrl = (url: string) =>
  `https://drive.google.com/viewerng/viewer?embedded=true&chrome=false&url=${url}`;
...
const pdfViewer = ({
  ...
  const injectedJS = `
    setTimeout(function() {
      // Intentar ocultar por clases conocidas de Google Drive Viewer
      var popoutButton = document.querySelector('.ndfHFb-c4YZDc-Wrql6b');
      if(popoutButton) {
        popoutButton.style.display = 'none';
      }
      var toolbar = document.querySelector('.ndfHFb-c4YZDc-GSQQnc-LgbsSe');
      if(toolbar) {
         toolbar.style.display = 'none';
      }
      // Ocultar cualquier enlace (<a>) en la barra superior que tenga target="_blank"
