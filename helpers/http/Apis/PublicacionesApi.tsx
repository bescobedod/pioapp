...
export interface PublicacionType {
  ...
  titulo: string;
  mensaje: string;
  estado: number; // 1 = Pendiente, 2 = Visto, 3 = Entendido
  createdAt: string;
...
export const getPublicacionesHistorial = async (
  id_categoria?: number
): Promise<ResponseService<PublicacionType[]>> => {
...
// Marcar como visto (Legacy)
export const postMarcarPublicacionLeida = async (
  id_publicacion: number
): Promise<ResponseService<any>> => {
  return await AJAX(`${URLPIOAPP}/publicaciones/leido`, 'POST', { id_publicacion });
};

// Actualizar el estado de la publicación (2 = Visto, 3 = Entendido)
export const postPublicacionEstado = async (
  id_publicacion: number,
  estado: number
): Promise<ResponseService<any>> => {
  return await AJAX(`${URLPIOAPP}/publicaciones/estado`, 'POST', { id_publicacion, estado });
};

// Obtener categorías
export const getCategoriasPublicacion = async (): Promise<
  ResponseService<CategoriaPublicacionType[]>
> => {
  return await AJAX(`${URLPIOAPP}/publicaciones/categorias`, 'GET');
};
